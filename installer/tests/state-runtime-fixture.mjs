...
import { loadState, saveState, clearState } from '../src/state.mjs';

const logPath = process.env.INSTALLER_RUNTIME_LOG;
const templateSource = process.env.INSTALLER_TEMPLATE_SOURCE;
const failAtStep = process.env.INSTALLER_FAIL_AT_STEP || '';
...
if (!templateSource) {
  throw new Error('INSTALLER_TEMPLATE_SOURCE is required for state runtime fixture');
}
...
function maybeFail(step) {
  if (failAtStep === step) {
    throw new Error(`intentional fixture failure at ${step}`);
  }
}
...
export const runtime = {
  ...
  async cloneRepo(installDir) {
    ...
    maybeFail('clone');
    await fsp.cp(templateSource, installDir, { recursive: true });
  },
  ...
  async saveState(completed, config) {
    ...
    return saveState(completed, config);
  },
  async clearState() {
