...
class OAuthTokenManager:
    ...
    def __init__(self, vault: Any, port: int = 8000, network_interceptor=None):
        ...
        self._state_file = Path(
            os.getenv("LANCELOT_ANTHROPIC_OAUTH_STATE_FILE", "/home/lancelot/data/anthropic_oauth_pending.json")
        )
    ...
    def _load_pending_flows(self) -> None:
        ...
        try:
            ...
            logger.warning("Failed to load Anthropic OAuth pending state: %s", exc)
            self._pending_flows = {}
    ...
    def _save_pending_flows(self) -> None:
        try:
            ...
            logger.warning("Failed to persist Anthropic OAuth pending state: %s", exc)
