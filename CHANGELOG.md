...
- docs: reconciled the remaining repo-wide local-model/runtime drift so README, security/governance/receipts/memory docs, legacy vNext specs, and CLAUDE workflow notes now match the live runtime. The repo now consistently documents the local model as the recommended privacy/cost boundary rather than an unconditional readiness requirement, and it explicitly describes the direct-frontier fallback behavior for reduced-footprint installs.

- local-model: wired the live `ModelRouter` into the gateway/orchestrator path so the local utility subsystem is no longer dead scaffolding. Frontier-bound user prompts and tool-result payloads are now scrubbed through the local redaction lane before `ProviderClient.generate()` / `generate_with_tools()` calls, HIVE decomposition uses the same scrubbed frontier wrapper, unified intent classification now stays on the local lane for obvious low-risk cases before any cloud fallback, and the control plane now reflects the real router-backed usage path instead of a standalone tracker detached from routing decisions.
- timetravel: replay and fork now fail closed when the governed quest executor is not wired, and `/api/timetravel/status` now exposes `quest_executor_ready` so Time-Travel can no longer look operational while its execution path is missing.
- mcp: `GovernedMCPProxy` now refuses to construct without the full guard stack (argument screener, response guard, and network policy), aligning constructor behavior with the documented fail-closed MCP governance contract.
- comms: wired the security bridge into the live Google Chat approval callback path so `/mcp_callback` can now authenticate bonded webhook traffic through `WebhookAuthenticator` instead of pretending to be an external callback while only accepting internal admin auth; added dedicated `LANCELOT_WEBHOOK_BEARER` support with `LANCELOT_API_TOKEN` fallback for backward compatibility.
- memory: episodic summarization now generates deterministic content-based archival summaries from the underlying memory items, replacing the old title-only placeholder output so the memory maintenance path matches the documented summarization behavior.

- control-plane: hardened `/system/pause` and `/system/emergency-stop` so empty request bodies fail cleanly with `400 Missing ... reason` instead of throwing a `500` JSON-parse error during live runtime control-plane use.

- docs: replaced the older production-hardening page with a real production-readiness guide covering auth mode selection, runtime control testing, federation/A2A go-live requirements, compliance export verification, staged rollout, and a concrete enterprise checklist; installation and quickstart now point operators to that guide before production use.

- Hardened HIVE, Time-Travel, and MCP status surfaces so War Room no longer gets healthy-looking defaults when those subsystems are uninitialized or internally failing. HIVE `/api/hive/status`, Time-Travel `/api/timetravel/status`, and MCP `/api/mcp/servers` now surface readiness, runtime degradation, and explicit error reasons instead of empty/default success-shaped payloads.

- compliance: upgraded SOC 2, ISO 27001, and GDPR exports from thin control-to-receipt dumps into auditor-grade evidence packages with explicit system context, export scope, integrity metadata, evidence-population summaries, operator-attribution summaries, exception summaries, separate legacy-attribution reporting, and framework-specific control/processing summaries while preserving receipt-level traceability.
- compliance: framework exports now download as ZIP audit bundles that contain the evidence JSON, a summary PDF, a flat CSV index, a README, and a manifest with per-file SHA-256 hashes, while the forensic timeline export remains a direct PDF artifact.
- Enterprise hardening:
  - Federation operator kills now run through the same persisted kill-switch engine as peer-issued kills: `/api/federation/manage/kill` issues the command into the durable ledger, executes the local leg, propagates to remote peers, and records remote acknowledgements/rejections into the shared command state instead of bypassing kill governance with a relay-only broadcast path.
  - Federation kill timeout progression is now live instead of dead code: reading command state advances overdue pending targets into `timeout`, so persisted commands no longer remain stuck in `propagating` forever after peers fail to respond.
  - Federation heartbeat subscription status is now connection-truthful instead of loop-truthful: runtime status and War Room now distinguish `connecting`, `connected`, `reconnecting`, `failed`, and `disconnected` based on actual SSE stream state instead of reporting a generic active subscription whenever a reconnect task exists.
  - Federation budget authority is now child-to-root only: non-root/peer instances reject inbound remote budget reports instead of allowing any known peer to drive local `hard_stop`, cost-report broadcasts now flow upward only to configured root peers, and the live federation daily ceiling now resolves from the deployed/active local graph node budget with `federation.yaml` fallback instead of a gateway hardcoded `$10/day`.
  - Federation divergence evaluation now fails closed instead of disappearing behind the heartbeat mesh: detector exceptions are logged, surfaced in `/api/federation/status` and `/api/federation/health` as degraded runtime state, and T3 HIVE spawns are blocked while divergence evaluation is unavailable instead of proceeding as if the federation were still connected.
  - Federation cost `HARD_STOP` no longer deadlocks the runtime-budget callback: federated threshold-change callbacks now run outside the aggregator lock, so live `hard_stop` transitions can read aggregate state, persist pause metadata, and drive the real runtime pause path instead of hanging the instance mid-transition.
  - Federation cost reporting now fails closed at the ingestion boundary: if the root-side federated budget aggregate cannot be updated, the incoming peer budget report is rejected instead of being acknowledged and dropped, and `/api/federation/budget` now surfaces aggregate computation failures directly instead of silently falling back to partial local-only data.
  - Federation inbound Soul push now validates sender hashes against the raw pushed Soul before local MCP ceiling narrowing, then recomputes the applied runtime hash from the narrowed document so legitimate root pushes no longer fail as false hash mismatches when the receiver enforces a stricter MCP ceiling.
  - Federation pause/resume now matches the root-governance model: receiving instances reject pause/resume commands from non-root peers, outbound pause propagation preserves the `full_stop` flag for T3/full-stop handling, and resume propagation no longer depends on a broken undefined payload field.
  - Federation T3/full-stop rollout completion is now truthful end to end: receiving peers send a real second-leg `/api/federation/soul/confirm` callback after local apply, root-side T3 resume waits on those confirmations, and local full-stop pauses now clear scheduler and Sentry approval queues while pausing active HIVE execution when available.
  - Federation budget `HARD_STOP` now drives the real persisted runtime pause path instead of only labeling the threshold as `pause_all_activity`, so chat, scheduler, inbound A2A, and other paused-runtime ingress points halt with the federated budget stop and remain operator-resume-only after pressure drops.
  - Federation T3 Soul rollout no longer auto-confirms or auto-resumes on push delivery: the runtime now leaves T3 propagations in `confirming`, exposes an explicit `/api/federation/soul/confirm` peer path, and resumes peers only after separate per-instance confirmation has been recorded.
  - Command Center pause is now a real persisted runtime control instead of a chat shortcut: War Room exposes `/system/pause` and `/system/resume`, the active pause state is visible in `/system/status`, and new chat, scheduler dispatch, HIVE task intake/spawn, inbound A2A execution, TaskRun execution, and federation-issued local pauses all read the same durable runtime pause state.
  - Command Center emergency stop is now a real control-plane action instead of a chat message: War Room exposes `/system/emergency-stop`, forces the runtime into paused state, and invokes the live local HIVE stop handler so active sub-agents are actually collapsed instead of only acknowledging the request.
  - Federation peer identity binding is now enforced at the runtime boundary: signed request identity is authoritative for kill, pause, handoff initiation, Soul update, and budget report handlers, and mismatched body-level peer IDs are now rejected instead of being trusted over the verified transport signature.
  - Federation pause commands now fail closed unless a real local pause engine is wired, and the live runtime now binds peer-issued Soul-propagation pauses to actual HIVE execution pauses instead of acknowledging a no-op local pause.
  - Federation heartbeat and divergence handling now use live runtime context instead of timestamp-only staleness: the heartbeat mesh passes current Soul hash, active task count, HIVE spawn count/state, pending handoffs, and budget utilization into the divergence detector, and peer registration/removal now notifies the heartbeat mesh immediately so new peer SSE subscriptions and removals do not wait for a process restart.
  - Federation divergence now reconciles in the live runtime instead of stopping at detection: reconnection events run the real compatibility check against the current Soul hash and budget state, incompatible reconciliations are preserved in detector state for operator follow-up, compatible reconciliations reset the mesh back to `connected`, and `/api/federation/status` now exposes live divergence plus reconciliation state.
  - Federation divergence and cost governance no longer fail open on restart: Soul propagation events and federated cost aggregates now persist to disk, stale peer cost pressure is removed when peers leave the mesh, and T3 HIVE spawns are blocked while the federation is actively diverged instead of proceeding with a status-only warning.
  - Federation cold-start/runtime truth is tighter: the heartbeat mesh now evaluates divergence immediately from persisted peer heartbeat state on startup, federated cost enforcement now treats stale peer budget snapshots as a hard governance gap instead of silently using old data, and pending mutual peer-registration handshakes now survive restart with bounded TTL.
  - Federation kill governance now matches the documented root-authority model: inbound federation kill commands are accepted only from peers registered as `role=root`, and the federated kill ledger now persists issued commands, ack/reject state, timeout progression, and lift-review state so active interventions survive restart instead of disappearing from process memory.
  - Federation operator status now fails closed instead of showing optimistic defaults: `/api/federation/status` and `/api/federation/health` now surface degraded runtime state, stale-cost peers, circuit-breaker summaries, heartbeat subscription status, and transport/mesh/reporter readiness instead of silently falling back to `synchronized` / `normal` / `connected` when federation runtime inspection fails, and Federation Overview now shows those degraded control-plane signals directly.
  - Federation budget threshold reporting no longer fails open on aggregate errors: `/api/federation/budget/threshold` now returns `503` with `threshold: unknown` when cost aggregation is unavailable, and federation status now treats explicit cost-reporter error payloads as degraded runtime state instead of quietly reporting a healthy control plane with an `unknown` threshold and no warning.
  - Federation Soul rollout and budget status are now live instead of hardcoded: the gateway instantiates the `SoulPropagationEngine`, `SoulTransport` records active T1/T2/T3 rollout state into that engine during real push flows, `/api/federation/status` now reports live `soul_consistency`, active propagation events, and the current cost threshold, and federation budget reporting is now wired to the orchestrator usage tracker plus HIVE active-spawn pressure instead of returning an empty shell when the subsystem was nominally enabled.
  - Federation T2/T3 rollout now fails closed on the required pause phase: if any target rejects or times out on `/api/federation/pause`, the runtime aborts before `/api/federation/soul/update`, records the propagation as failed, and skips `/api/federation/resume` instead of pushing Soul changes through a rejected full-stop boundary.
  - Federation T3 rollout completion no longer overstates success on failed resume: after per-instance confirmations, a rejected `/api/federation/resume` leg now leaves the propagation failed instead of marking the event `completed` while one or more peers remain paused.
  - Federated kill execution now fails closed for the local instance: if the local kill handler is missing or throws, the kill ledger records the local target as rejected and the command resolves to `failed` or `partial` instead of acknowledging a kill that never executed.
  - Federation heartbeat status now preserves last stream outcome alongside the reconnect lifecycle: runtime status and Federation Overview expose both subscription state and the last stream result/error, so operators can distinguish “reconnecting after HTTP/connect failure” from a healthy active stream instead of losing the failure signal behind a generic `reconnecting` label.
  - Federation contradiction governance now fails closed: if the contradiction detector is configured but unavailable, inbound handoff initiation and completion are rejected instead of proceeding as if no contradictions existed, and top-level federation status now treats failed heartbeat streams as degraded runtime state instead of relegating that signal to a detail-only field.
  - Federation Soul availability is now treated as a real runtime prerequisite instead of an empty-string default: `/api/federation/status` degrades when the live runtime Soul hash is unavailable, and `/api/federation/soul/hash` now resolves from the live Soul transport and returns `503` instead of `200` with an empty hash when no active runtime Soul is available.
  - A2A management status no longer fails open on internal subsystem errors: `/api/a2a/status` now surfaces degraded runtime state, dependency readiness, and explicit registry/Soul errors instead of quietly reporting `enabled` with zero agents when the live registry or Soul resolver is broken.
  - Federation budget governance is now enforced on the real HIVE spawn path: live threshold changes emit federation threshold receipts and audit entries, HIVE sub-agent spawns are blocked when federation cost governance enters gated states, local spawn-budget ceilings are checked before registration, and spawn/collapse events update the live federation budget tracker instead of leaving enforcement as status-only dead code.
  - Federation handoff contradiction enforcement is now live instead of dormant: receipt chains are checked for temporal contradictions, completion payloads are validated against declared handoff schemas and bounds, failed completions with declared success criteria are rejected as contradictions, and contradiction events are now written into the federation audit timeline instead of being silently ignored.
  - Federation Soul transport is now a live runtime path instead of a placeholder: `/api/federation/soul` returns the active runtime Soul document and deterministic hash, inbound Soul pushes validate/lint/hash-check the incoming document before persisting and refreshing the live runtime, heartbeat emissions refresh immediately after Soul changes, and federation handshake comparisons now compute real compatibility metadata instead of always returning success.
  - Federation Soul push completion now reports real peer acknowledgement state: live rollout responses are summarized through the Soul handshake model, governance gaps and timeouts are surfaced in push results and audit details, and stale propagation events are finalized instead of staying in an indeterminate active state.
  - Federation Soul propagation now respects the intended authority boundary: only peers registered as `root` can push runtime Soul updates, inbound MCP ceiling narrowing now uses the receiver's live Soul as the actual ceiling instead of comparing the pushed document against itself, the public `/api/federation/soul/handshake` endpoint now runs the real compatibility evaluation path, and T2/T3 rollouts now issue an explicit resume phase after pause and update delivery.
  - Federation kill handling now fails closed unless a local kill engine is wired, and the live federation gateway now routes incoming kill commands through a real local kill engine backed by HIVE `kill_all` instead of acknowledging commands that performed no local action.
  - Federation runtime truth now survives restart: audit timelines persist to disk, active handoff status survives fresh protocol instances instead of disappearing from memory, and peer heartbeat freshness plus live Soul hash updates are written back into the persisted topology state instead of being lost on reload.
  - Federation replay protection is now restart-stable: signed peer request verification records nonces in the persistent federation peer registry instead of an in-memory cache only, replay rejection survives fresh auth instances and normal process restarts, and old durable nonces are pruned on a bounded interval during live verification.
  - Federation peer bootstrap now completes the documented mutual confirm handshake instead of a one-shot trust update: `/peer/confirm` is live, known peer identities are pinned against silent key/address takeover, caller fingerprints are recomputed from submitted keys, and outbound registration now requires a configured `self_address` so the callback leg can finish before either side persists the peer.
  - Federation local-instance configuration is now operable from War Room: Federation Overview exposes a `This Instance` card for `self_address`, instance ID, fingerprint, and public key, and Graph Builder can prefill the local node from that canonical identity source instead of requiring duplicate manual entry.
  - Communications now fail closed to the runtime contract: unsupported `LANCELOT_COMMS_TYPE` values no longer silently boot the Google Chat backend, onboarding only offers the bidirectional comms backends that actually exist (`telegram`, `google_chat`, or `none`), and the Google Chat poller now reads the live configured space, sends orchestrator responses back to Chat, and suppresses self-loop reprocessing with a bounded sent-message cache.
  - MCP/Sentry approval state now persists across restarts instead of existing only in memory, and the security bridge no longer defaults outbound comms to Google Chat when no runtime comms backend is configured.
  - Restart-sensitive approval and protocol state now survives process restarts: scheduler pending/granted approvals persist to disk, inbound A2A task status/artifact state persists across server reinitialization, and MFA challenges now use a bounded persistent store instead of disappearing on restart.
  - Browser and enterprise auth continuity no longer depend on process memory alone: War Room sessions and OIDC pending/exchange state now persist across restart, and the health snapshot now reports real scheduler liveness plus `last_scheduler_tick_at` instead of marking the scheduler healthy based only on object presence.
  - Observability and external-auth workflows now survive restart correctly: webhook retry queues persist instead of being dropped from memory, Google/Anthropic/Codex PKCE handshakes survive callback windows across restart, and UCP pending transactions remain confirmable after process recreation.
  - Raw `gateway.py` control routes now enforce coarse operator capabilities instead of broad `verify_token()` access for privileged actions such as secret reload, Crusader toggles, UCP mutations, Google OAuth credential lifecycle, MCP callback approval, and Forge operations.
  - Inbound A2A task handling no longer reports false success from a completion stub. Accepted external tasks now execute through an injected runtime callback and only return `completed` when execution actually succeeds.
  - TaskRun verification now fails closed when `requires_verifier=True`: missing verifiers and verifier exceptions block the run instead of silently passing verification.
  - Connector startup now fails safe: the governed connector runtime synthesizes a fallback risk classifier when the orchestrator classifier is unavailable, and connector management/credential routers mount independently so the entire subsystem no longer disappears behind a startup warning.
  - Tasking and scheduling now fail closed when no real executor path exists: TaskRun no longer reports placeholder success for `FILE_EDIT`, `COMMAND`, or `TOOL_CALL` steps without a live executor, and scheduled jobs no longer emit `scheduled_job_run` when the scheduler has no skill executor configured.
  - War Room artifacts now route through a bounded structured store with server-side normalization, trusted artifact-type validation, admin-only manual writes, and server-stamped session/operator/source metadata instead of accepting arbitrary client JSON into an unbounded global list.
  - MFA challenges are now bound to the originating operator/session, and foreign sessions can no longer release another operator's pending MFA challenge.
  - Scheduler approval API now propagates the authenticated operator identity into approval receipts instead of recording anonymous approvals.
  - HIVE task intake, manual scheduler triggers, federation handoff completion, and governed MCP tool receipts now preserve authenticated operator/session or signed-peer provenance end to end instead of dropping identity at the execution edge.
  - HIVE child-agent receipts now preserve the initiating operator/session identity all the way down the agent DAG: spawned, state-transition, governance-check, action, pause/resume, and collapse receipts now inherit the top-level operator provenance instead of falling back to anonymous agent-only events.
  - Governance receipt emission is restored for shared admin helpers, and outbound A2A delegations from the management API now carry authenticated operator/session identity through the live pipeline and receipt chain.
  - Webhook bearer verification now fails closed when no explicit shared secret is configured.
  - Chat execution provenance is now preserved end to end: the orchestrator accepts explicit session/operator/quest context, War Room chat routes forward authenticated operator identity into `chat()`, inbound A2A execution preserves the quest allocated by the inbound pipeline, and `LLM_CALL` receipts now carry operator/session metadata instead of always starting a fresh anonymous quest.

- actioncards/time-travel: wired ActionCard approvals into the real Soul, governance, scheduler, and skills mutation paths with authenticated operator/session propagation, and upgraded Time-Travel replay/fork from receipt-only quest minting to actual TaskRun-backed re-execution under the live runtime Soul.
- soul/a2a: hardened live governance consistency so Soul activation now refreshes the in-memory runtime instead of only flipping files on disk, the risk classifier/HIVE/MCP/Time-Travel subscribers now follow the active runtime Soul, outbound A2A now enforces real network allowlist host matching, and Agent Card verification now behaves like a pinned trust check with origin/auth/framework validation and explicit operator re-verification before card drift is accepted.
- connectors/tasking/a2a: wired the governed connector runtime into the live gateway so connector operations can execute through `ConnectorProxy` and `GovernedConnectorProxy` with risk classification, policy checks, and receipt emission; narrowed TaskRun execution authority from generic step classes to concrete tools, skills, and file paths; hardened TaskRun receipt lineage with operator/session/quest/parent linkage; and switched A2A inbound, outbound, status, and Agent Card generation to resolve permissions from the live active Soul instead of a startup snapshot.
- a2a: hardened the external-agent boundary so inbound A2A now accepts only preregistered bearer-token or API-key peers, rejects unknown callers before registration, verifies pinned Agent Cards before allowing inbound work, binds task status/SSE reads to the same authenticated peer that submitted the task, and routes outbound delegation through the real governed A2A client with vault-backed credential resolution, Agent Card verification, and polling instead of the previous stub path.
- hive/federation: hardened bounded-delegation enforcement so HIVE startup now wires parent Soul plus governance into live sub-agent runtimes, scoped Souls are rejected if they are not provably more restrictive than the parent, runtime action loops collapse on scoped-Soul boundary violations, HIVE UAB execution enforces scoped app/category boundaries before desktop actions, and federation handoff acceptance now validates incoming task payload schema, Soul compatibility, Soul intersection monotonicity, and contract-level Soul constraints before accepting work.
- security: hardened the Incident Response playbook reload path so `POST /api/playbooks/reload` now requires `incidents.admin` instead of any authenticated operator.
- observability: fixed Metrics API runtime wiring so the router mounts with Observability and obeys live config toggles, added `observability.admin` authorization, enforced per-operator rate limiting, and restored optional `METRICS_API_QUERY` receipt emission for detailed receipt lookups.
- control-plane: restricted `POST /api/updates/check` and `POST /api/updates/dismiss` to `platform.admin` since both mutate shared process state and `check` forces outbound version fetches.
- incident-response: `POST /api/playbooks/reload` now emits the documented `PLAYBOOK_UPDATED` receipt so playbook registry changes are visible in governance, webhooks, and compliance export trails.
- api-contracts: removed stale client-supplied actor fields from Incident, Memory, and Skills request schemas so OpenAPI matches the hardened server-side identity model.

### Fixed - Control-plane authorization hardening
- **Authenticated onboarding mutations** (`src/core/control_plane.py`, `src/core/auth_api.py`): Onboarding state-changing endpoints now require both backend authentication and the `onboarding.admin` capability, closing the previously live unauthenticated reset and command paths.
- **Coarse admin capability model** (`src/core/auth_api.py`, `src/core/providers/api.py`, `src/core/setup_api.py`, `src/core/flags_api.py`, `src/core/soul/api.py`): Browser and OIDC sessions now carry explicit capabilities, OIDC admin elevation is driven by `OIDC_ADMIN_GROUPS`, and privileged provider/setup/flags/Soul routes now enforce admin capabilities instead of treating all authenticated users as full operators.
- **Canonical auth module resolution** (`src/core/gateway.py`, `src/core/setup_api.py`, `src/core/warroom_ws.py`): Removed legacy mixed `auth_api` import paths so runtime auth, websocket auth, and audit attribution all resolve through the same `src.core.auth_api` module.
- **Authorization regression coverage** (`tests/test_auth_api.py`, `tests/test_control_plane_auth_hardening.py`, `tests/test_setup_api.py`, `tests/test_admin_authorization_hardening.py`): Added focused tests for session capabilities, non-admin denial on onboarding/setup/provider/Soul surfaces, and OIDC group-to-capability mapping.
- **Additional admin-surface capability gates** (`src/core/governance_api.py`, `src/core/trust_api.py`, `src/core/apl_api.py`, `src/core/skills_api.py`, `src/core/connectors_api.py`, `src/connectors/credential_api.py`, `src/observability/api.py`, `src/incidents/api.py`, `src/core/memory/api.py`, `src/core/soul/template_api.py`): Governance approvals, trust/APL mutations, skills management, connector and credential management, observability control, incident management, governed memory writes, and Soul template application now require explicit admin capabilities instead of generic authenticated access.
- **Operator identity integrity** (`src/core/auth_api.py`, `src/incidents/api.py`, `src/core/memory/api.py`, `src/core/soul/template_api.py`, `src/core/skills_api.py`): Incident receipts/timelines, governed memory commits/actions, Soul template application, and skill approvals now derive actor identity from the authenticated session or API key instead of trusting client-supplied `operator_id`, `created_by`, `approver`, or `approved_by` fields.
- **Federation, HIVE, Compliance, and BAL authorization alignment** (`src/core/auth_api.py`, `src/federation/api.py`, `src/federation/graph_api.py`, `src/hive/api.py`, `src/compliance/api.py`, `src/core/bal/clients/api.py`): Added `federation.admin`, `hive.admin`, `compliance.admin`, and `bal.admin` capability gates across the remaining privileged subsystem routes, and Graph Builder now derives topology authorship and yellow-edge acknowledgments from the authenticated operator instead of trusting client-supplied identity fields.
- **Federation data-plane auth contract repair** (`src/federation/api.py`, `src/federation/heartbeat_mesh.py`, `src/core/gateway.py`, `tests/federation/test_api.py`): Federation peer bootstrap and signed inter-instance transport routes no longer sit behind War Room session auth. Peer registration now reaches federation bootstrap logic without browser credentials, signed federation command/stream routes verify federation request signatures instead of generic UI auth, and the heartbeat mesh now signs its stream subscription requests.
- **Canonical feature-flag module state** (`src/core/feature_flags.py`, `tests/test_subsystem_runtime_contract.py`): Normalized `feature_flags` and `src.core.feature_flags` to the same module object so runtime kill switches, route gates, and subsystem checks all observe the same live flag state. This closes cases where a flag toggle updated one import path while request-time middleware read another.
- **ActionCard approval hardening** (`src/core/actioncard_api.py`, `src/core/actioncard/resolver.py`, `src/warroom/src/api/actioncards.ts`, `tests/test_admin_authorization_hardening.py`): ActionCard listing and resolution now require `platform.admin`, resolution receipts carry the authenticated operator identity, and the War Room API client now matches the backend route contract for pending-card fetches and button resolution.
- **HIVE audit attribution cleanup** (`src/hive/api.py`): HIVE intervention audit logs now record the authenticated operator display name or operator ID instead of the stale hardcoded `WarRoom` pseudo-user.
- **Scheduler and control-plane authorization tightening** (`src/core/auth_api.py`, `src/core/scheduler_api.py`, `src/core/control_plane.py`, `tests/test_admin_authorization_hardening.py`, `tests/test_control_plane_auth_hardening.py`): Scheduler management now requires `scheduler.admin`, and the control-plane usage reset plus execution-token list/get/revoke routes now require `platform.admin` instead of generic authenticated access.
- **A2A, MCP, and Time-Travel admin gates** (`src/core/auth_api.py`, `src/a2a/api.py`, `src/mcp/api.py`, `src/timetravel/api.py`, `tests/test_admin_authorization_hardening.py`): Internal A2A management, MCP server management, and Time-Travel inspection/replay/fork endpoints now require explicit subsystem admin capabilities instead of generic authenticated access.

### Fixed - Callback and operator-attribution hardening
- **OAuth callback XSS remediation** (`src/core/gateway.py`, `src/core/oauth_callback_pages.py`): Anthropic, Codex, and Google OAuth browser callbacks now escape `error_description` content and return generic failure pages on internal exceptions instead of reflecting raw exception text into HTML.
- **Authenticated identity propagation** (`src/timetravel/api.py`, `src/a2a/api.py`, `src/core/auth_api.py`): Time-Travel replay/fork and manual A2A governance operations now resolve operator identity from the authenticated War Room session or API-key identity instead of requiring legacy `X-Operator-ID` headers.
- **Soul proposal authorship integrity** (`src/core/soul/api.py`): Soul amendment proposals now record the authenticated operator as the author and ignore client-supplied `author` fields.
- **Audit log attribution cleanup** (`src/core/flags_api.py`, `src/core/gateway.py`): Flag toggles and Crusader API actions now log the resolved authenticated operator instead of hardcoded pseudo-users like `WarRoom` or `Commander`.

### Changed - Memory tier wiring hardened to match architecture
- **Archival promotion fixed** (`src/core/memory/jobs.py`): Episodic summarization now writes generated summaries into the archival tier, not back into episodic storage, and marks summarized episodic items deprecated.
- **Scheduler integration for memory maintenance** (`src/core/gateway.py`, `src/core/scheduler/service.py`, `src/core/memory/jobs.py`): Built-in memory maintenance jobs are now ensured in the live scheduler registry and can execute even when no general skill executor is present.
- **Working-memory producer activated** (`src/core/memory/compiler.py`, `src/core/orchestrator.py`): The orchestrator now records the active objective into quest-scoped working memory with TTL-backed expiry before context compilation, so the working tier is live rather than dormant.
- **Documentation alignment** (`docs/memory.md`, `docs/architecture.md`): Updated the memory architecture docs to match the live tier wiring, maintenance jobs, and active-objective working-memory behavior.

### Changed - Frontend dependency hardening
- **War Room npm audit remediation** (`src/warroom/package-lock.json`): Refreshed the frontend lockfile to resolve the previously flagged high-severity advisories in `vite`, `rollup`, `picomatch`, and transitive `lodash`, bringing the War Room dependency graph back to a clean `npm audit` state without widening the application dependency surface.

### Changed - Authentication hardening and enterprise auth planning
- **War Room password storage** (`src/core/auth_api.py`, `src/core/secret_cache.py`): New and migrated local War Room passwords now use bcrypt instead of unsalted SHA-256. Plaintext env passwords are converted to bcrypt on vault migration, while legacy SHA-256/plaintext values remain readable only long enough for successful login to transparently upgrade them.
- **Login throttling** (`src/core/auth_api.py`): Failed `/auth/login` attempts are now rate-limited per client IP, returning `429` after repeated failures within a sliding window.
- **Cookie-backed War Room sessions** (`src/core/auth_api.py`, `src/core/gateway.py`, `src/core/warroom_ws.py`, `src/warroom/src/api/auth.ts`, `src/warroom/src/api/client.ts`): Browser auth now uses server-issued HttpOnly session cookies instead of bearer tokens in `localStorage`, `/war-room` no longer injects an API token into the SPA HTML, file downloads use authenticated cookies, and War Room WebSocket auth accepts the browser session cookie.
- **Auth dependencies** (`pyproject.toml`, `requirements.txt`): Added explicit `bcrypt` dependency for local password hardening.
- **Authentication architecture** (`docs/authentication.md`, `docs/architecture.md`): Added the enterprise-capable auth design covering local mode, enterprise OIDC SSO, break-glass admin access, coarse capability mapping, and governance identity requirements.
- **Installer + onboarding auth selection** (`installer/src/index.mjs`, `installer/src/config.mjs`, `src/ui/onboarding.py`): Fresh installs and in-app onboarding now choose an explicit War Room auth model: local account or enterprise OIDC. Local setup collects username/password and generates a one-time password reset code. Enterprise setup collects issuer/client credentials and writes OIDC env config instead of local credentials.
- **War Room login branching** (`src/core/auth_api.py`, `src/warroom/src/pages/LoginPage.tsx`, `src/warroom/src/pages/LoginCallbackPage.tsx`): The login screen now loads backend auth configuration, shows local sign-in plus local password reset only when local auth is active, and switches to an enterprise SSO button with OIDC callback/exchange flow when OIDC is configured.

### Changed - UAB standalone merge and host persistence
- **Embedded standalone runtime** (`packages/uab/src/daemon.ts`, `packages/uab/src/server.ts`, `packages/uab/src/connector.ts`, `src/tools/providers/uab_bridge.py`, `src/core/flags_api.py`): Lancelot's internal UAB now fronts the standalone 1.3.0 core through a JSON-RPC compatibility daemon on `:7900`, preserving the governed Python bridge while exposing newer connector-backed methods and War Room status metadata (`transport`, `standalone_features`).
- **Windows persistence path** (`scripts/install-uab.bat`, `scripts/start-uab.bat`, `scripts/run-uab-daemon.bat`): The Windows installer now uses a stable launcher script for the `LancelotUABDaemon` Scheduled Task, fixes Node.js version detection, and starts the compatibility daemon reliably even when the repo path contains spaces.
- **Docs and operations** (`docs/uab.md`, `docs/architecture.md`, `docs/operations/runbooks/uab.md`, `README.md`): Updated the UAB architecture, runbook, and README to reflect the standalone-core merge, the JSON-RPC compatibility layer, the new War Room status fields, and the persistent Windows install path.

### Added — OpenAI Codex OAuth Provider
- **OpenAI Codex OAuth Manager** (`src/core/openai_codex_oauth_manager.py`): Full PKCE OAuth 2.1 flow for authenticating via ChatGPT Plus/Pro subscription. Vault-backed token storage (`openai.codex.access_token`, `openai.codex.refresh_token`, `openai.codex.token_expiry`, `openai.codex.account_id`) with background refresh and restart recovery.
- **CLI-backed Codex provider** (`src/core/providers/codex_cli_client.py`): `openai-codex` now runs through the official `codex exec` client inside the container instead of making direct model API calls with the ChatGPT OAuth token. This avoids the `missing_scope: model.request` failure path and returns the known GPT-5.4 lane set locally.
- **Governed Codex tool bridge** (`src/core/providers/codex_cli_client.py`, `src/core/orchestrator.py`, `src/core/orch_helpers/intent_helpers.py`): Codex now proposes normalized Lancelot tool calls through a schema-driven planner bridge. Execution still happens inside Lancelot's governed agentic loop, so safety classification, approval/Sentry checks, receipts, and skill execution remain under Lancelot control.
- **Container support** (`Dockerfile`, `docker-compose.yml`, `entrypoint.sh`): Installs `@openai/codex`, mounts host `~/.codex` into `/home/lancelot/.codex`, and exposes port `1455` for the OAuth callback.
- **Provider factory and orchestrator** (`src/core/providers/factory.py`, `src/core/orchestrator.py`): Added `openai-codex` as an OAuth-only provider, improved startup recovery messaging, and hot-swapped provider recovery after OAuth completes.
- **Gateway integration** (`src/core/gateway.py`): Added `/auth/callback`, post-startup Codex provider recovery, fresh `/health/ready` snapshots, and fixed optional receipt-service wiring for Time-Travel, A2A, and Incident Response startup.
- **Provider API and onboarding** (`src/core/providers/api.py`, `src/ui/onboarding.py`, `src/warroom/src/api/providers.ts`, `src/warroom/src/pages/CostTracker.tsx`): Added initiate/status/revoke endpoints and War Room setup flows for Codex OAuth.

### Fixed — Codex/War Room Integration
- **Health check prompt auth failure**: Health-check and normal chat prompts no longer attempt ChatGPT OAuth requests against operations that require `model.request`.
- **Codex tool-use enforcement**: Live status and health-check prompts now classify as tool-backed research, first-iteration `mode=ANY` is enforced by schema, and Codex cannot satisfy those requests with an unguided text-only completion.
- **Startup noise** (`src/core/update_checker.py`, `src/core/gateway.py`, `src/core/orchestrator.py`): DNS-only version-check failures now log at info, UAB startup unavailability is downgraded to info, and the misleading `No API key for openai-codex` startup line now reports missing Codex OAuth recovery state accurately.
- **War Room chat scroll behavior** (`src/warroom/src/pages/command/ChatInterface.tsx`): Enter no longer grows the command panel and pushes the page; the message area now has a fixed responsive panel height with isolated internal scrolling.

### Added — UAB v0.6.0 Feature Parity Upgrade
- **Browser Plugin** (`packages/uab/src/plugins/browser/index.ts`, 1469 lines): Full CDP control of Chrome, Edge, Brave, Vivaldi, Opera — tab management, cookies (CRUD), localStorage/sessionStorage, navigation, JS execution, screenshots. Auto-discovers debug port or relaunches browser with `--remote-debugging-port`.
- **Chrome Extension Plugin** (`packages/uab/src/plugins/chrome-ext/`, 3 files, 940 lines): WebSocket bridge (port 8787) to Manifest V3 Chrome extension for zero-relaunch browser control. Includes CRX packaging and Windows registry installer.
- **Vision Plugin** (`packages/uab/src/plugins/vision/`, 3 files, 1216 lines): Claude Vision API fallback (claude-sonnet-4-20250514) for universal app coverage. Screenshot → element detection → coordinate-based input injection via Win32 P/Invoke. Lazy SDK import — graceful fallback if `@anthropic-ai/sdk` not installed.
- **App Registry** (`packages/uab/src/registry.ts`, 296 lines): In-memory Map + JSON persistence (`data/uab-profiles/registry.json`). Dual-indexed by executable and PID, remembers preferred control methods across sessions.
- **Spatial Map Engine** (`packages/uab/src/spatial.ts`, 567 lines): Converts UIElement trees into spatial index with row detection (Y-clustering, 15px threshold) and grid-cell indexing (50px cells). `renderTextMap()` generates AI-consumable text representation that replaces screenshots.
- **Composite Query Engine** (`packages/uab/src/composite.ts`, 358 lines): Combines UIA tree + bounding rects + text reading + optional vision in speed-priority order. Methods: `query()`, `quickMap()`, `textMap()`, `findElement()`, `clickElement()`, `typeInto()`.
- **UABConnector** (`packages/uab/src/connector.ts`, 1340 lines): Public API wrapper combining all modules. Advanced methods: `focused()`, `findByPath()`, `watchChanges()`, `atomicChain()`, `smartInvoke()`. Extension bridge lifecycle, registry integration.
- **MCP Server** (`packages/uab/src/mcp-server.ts`, 851 lines): 17 MCP tools over stdio for Claude Desktop integration — `desktop_scan`, `desktop_apps`, `desktop_connect`, `desktop_ui_tree`, `desktop_find_elements`, `desktop_act`, `desktop_spatial_map`, `desktop_state`, `desktop_keypress`, `desktop_hotkey`, `desktop_window`, `desktop_smart_click`, `desktop_chain`, `desktop_focused`, `desktop_flow`, `desktop_deep_query`, `desktop_invoke`.
- **AgentSDK** (`packages/uab/src/sdk.ts`, 246 lines): High-level API — `look()`, `click()`, `type()`, `shortcut()`, `key()`, `find()`, `status()`, `apps()`, `do(steps)`. Pre-configured `desktop` singleton.
- **Agent Prompt Templates** (`packages/uab/src/agent-prompt.ts`, 258 lines): System prompts for MCP/CLI/HTTP/Core modes teaching agents to prefer structured data over screenshots. CLAUDE.md snippet generator and MCP config generator.
- **Environment Detection** (`packages/uab/src/environment.ts`, 196 lines): Auto-detects runtime mode (desktop/server/container) and adapts defaults (rate limits, cache TTLs, extension bridge).
- **Flow Library** (`packages/uab/data/flow-library/`, 13 JSON files): Pre-built interaction flows for ChatGPT, Grok, Excel, Notepad, Slack, Discord, Outlook, Blender, CapCut, FreeCAD.
- **27 new ActionTypes** (`packages/uab/src/types.ts`): Browser session/cookie actions — `navigate`, `goBack`, `goForward`, `reload`, `getTabs`, `switchTab`, `closeTab`, `newTab`, `executeScript`, cookie CRUD (4), localStorage CRUD (4), sessionStorage CRUD (4). Total ActionTypes: 61 (was 34).
- **New type interfaces**: `FocusedElementInfo`, `PathSelector`, `StateChangeEvent`, `AtomicChainDef`, `AtomicChainResult`, `SmartResolveResult`.
- **Python bridge v0.6.0** (`src/tools/providers/uab_bridge.py`): New methods — `spatial_map()`, `text_map()`, `find_by_description()`, `navigate()`, `get_tabs()`, `switch_tab()`, `execute_script()`, `get_cookies()`, `set_cookie()`, `get_local_storage()`, `set_local_storage()`.
- **Daemon v0.6.0** (`packages/uab/src/daemon.ts`): New JSON-RPC methods — `spatialMap`, `textMap`, `findByDescription`. Version bumped to 0.6.0. Browser framework added to supported list.

### Changed — UAB v0.6.0
- `packages/uab/src/router.ts`: Vision fallback now implemented (was "not yet implemented"). `getAvailableMethods()` adds `'vision'` as last fallback.
- `packages/uab/src/service.ts`: Registers BrowserPlugin and VisionPlugin. Integrates CompositeEngine with new methods `spatialMap()`, `textMap()`, `findByDescription()`.
- `packages/uab/src/index.ts`: Exports all new modules — UABConnector, AppRegistry, SpatialIndex, CompositeEngine, AgentSDK, environment, plugins.
