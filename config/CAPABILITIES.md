...
- Multi-provider: Supports Google Gemini, OpenAI, Anthropic, xAI (Grok), NVIDIA Nemotron, DeepSeek, and operator-managed OpenAI-compatible local model endpoints as LLM backends
- Provider is configured via LANCELOT_PROVIDER env var (gemini, openai, anthropic, xai, nvidia, deepseek, local-openai)
...
- Provider-agnostic: works with Gemini, OpenAI, Anthropic, xAI, NVIDIA, DeepSeek, or local OpenAI-compatible providers (uses active LANCELOT_PROVIDER)
