...
Governed model/tool turns first persist a `waiting_worker_slot` progress event and remain queued until the async worker slot is acquired. This keeps Command Center from presenting a queued turn as active execution while it is waiting behind earlier governed work.
...
- `wait_reason`: short machine-readable wait class for operator display. Current values include `worker_slot`, `provider_call`, `approval`, and `finalization`.
...
Command Center must also render wait reasons as plain-language status. A user should be able to distinguish "waiting for a governed worker slot" from "waiting on the provider", "waiting for Commander approval", and "finalizing response/proof" without reading logs.
