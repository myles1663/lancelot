...
| `FEATURE_MEMORY_VNEXT` | true | Tiered memory (persona, human, mission, workspace) |
...
| Flag | Default | Description |
|------|---------|-------------|
| `FEATURE_CONNECTORS` | true | External connector system. *Requires: FEATURE_TOOLS_FABRIC* |
| `FEATURE_TRUST_LEDGER` | true | Progressive tier relaxation. *Requires: FEATURE_RISK_TIERED_GOVERNANCE* |
| `FEATURE_SKILL_SECURITY_PIPELINE` | true | 6-stage security pipeline for new skills. *Requires: FEATURE_SKILLS* |
| `FEATURE_APPROVAL_LEARNING` | true | Learn owner decision patterns. *Requires: FEATURE_RISK_TIERED_GOVERNANCE* |

### Intelligence & Reasoning

| Flag | Default | Description |
|------|---------|-------------|
| `FEATURE_STRUCTURED_OUTPUT` | true | JSON schema output with receipt verification. *Requires: FEATURE_AGENTIC_LOOP* |
| `FEATURE_CLAIM_VERIFICATION` | true | Cross-reference response claims vs tool receipts |
| `FEATURE_UNIFIED_CLASSIFICATION` | true | Single LLM call for intent routing |
