...
def provider_enabled():
    ...
    backend = MagicMock()
    backend.provider_name = "openai"
    backend.model_name = "gpt-5.4-mini"
    backend.lane = "fast"
    backend.client.complete.return_value = json.dumps(
        {
            "template_id": "antigravity:test1234",
            "project_summary": "A generated FastAPI service",
            "files": [
                {
                    "path": "main.py",
                    "content": (
    ...
    p._generation_backend = backend
    p._resolve_generation_backend = MagicMock(return_value=backend)
...
class TestHealthCheck:
    ...
    def test_health_service_available(self, provider):
        ...
        with patch("src.tools.providers.ui_antigravity.FEATURE_TOOLS_ANTIGRAVITY", True):
            ...
            backend.provider_name = "openai"
            backend.model_name = "gpt-5.4-mini"
            provider._generation_backend = backend
            with patch.object(provider, "_check_antigravity_service", return_value=True):
                ...
                assert health.metadata["generation_provider"] == "openai"
    ...
    def test_health_service_unavailable(self, provider):
        ...
        with patch("src.tools.providers.ui_antigravity.FEATURE_TOOLS_ANTIGRAVITY", True):
            with patch.object(provider, "_check_antigravity_service", return_value=False):
                ...
                assert "generation backend" in health.degraded_reasons[0].lower()
...
class TestGenerativeMode:
    ...
    def test_generative_when_available(self, provider_enabled, temp_workspace, sample_spec):
        ...
        assert result.success is True
        assert result.template_id == "antigravity:test1234"
        provider_enabled._generation_backend.client.complete.assert_called_once()
    ...
    def test_generative_creates_files(self, provider_enabled, temp_workspace, sample_spec):
        ...
        assert os.path.exists(os.path.join(temp_workspace, "README.md"))
    ...
    def test_generative_rejects_path_traversal(self, provider_no_fallback, temp_workspace, sample_spec):
        """GENERATIVE mode fails closed on unsafe model output."""
        provider_no_fallback._antigravity_available = True
        backend = MagicMock()
        backend.provider_name = "openai"
        backend.model_name = "gpt-5.4-mini"
        backend.lane = "fast"
        backend.client.complete.return_value = json.dumps(
            {
                "template_id": "antigravity:unsafe",
                "project_summary": "unsafe",
                "files": [
        ...
        provider_no_fallback._generation_backend = backend
        provider_no_fallback._resolve_generation_backend = MagicMock(return_value=backend)
...
class TestReceiptGeneration:
    ...
    def test_generative_creates_receipt(self, provider_enabled, temp_workspace, sample_spec):
        ...
        assert receipts[0]["provider_name"] == "openai"
        assert receipts[0]["model_name"] == "gpt-5.4-mini"
...
class TestIntegration:
    ...
    def test_generation_parser_accepts_fenced_json(self, provider):
        """Generation parser accepts fenced JSON responses."""
        payload = provider._parse_generation_response(
            """```json
{"template_id":"antigravity:test","files":[{"path":"README.md","content":"# ok"}]}
```"""
        )

        assert payload["template_id"] == "antigravity:test"
